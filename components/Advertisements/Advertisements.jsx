  ...
  return (
    <>
      ...
      >
        {adsToShow.map((ad) => (
          <Link
            href={`/${locale}/ads/${ad.root_category_slug}/${ad.id}`}
            key={ad.id}
            className="relative flex items-center flex-col rounded-4xl bg-surface space-y-4 cursor-pointer pb-6 hover:shadow-lg transition-shadow duration-300"
          >
            {ad.is_verified !== 0 && (
              <div className="absolute py-1 px-2 top-2 right-4 rounded-lg bg-[#DCFCE8] flex flex-row gap-1 z-10">
            ...
            <h2 className="font-[500] text-lg text-right self-end px-4 line-clamp-2">
              {ad.title}
            </h2>
