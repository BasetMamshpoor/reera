...
const shabaSchema = z.object({
  shaba: z
    ...
    .refine((shaba) => isValidIranianShaba(shaba), "شماره شبا معتبر نیست"),
});
  ...
  return (
    <div className="flex flex-col gap-6 justify-between h-full p-8">
      <div className="flex flex-row w-full justify-between items-center">
        <h2 className="text-xl font-semibold">افزودن شماره شبا</h2>
        <Close
          onClick={() => {
      ...
      <span className="w-full max-w-md text-lg text-Text-Secondary">
        لطفا شماره شبا (IBAN) خود را وارد کنید. شماره شبا باید با IR شروع شود و
        26 کاراکتر داشته باشد.
      </span>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-6">
        <div className="space-y-2">
          <Input
            type="text"
            placeholder="شماره شبا (مثال: IR060510000000100248240001)"
            className={`py-6 border ${
              errors.shaba ? "border-error-main" : "border-default-divider"
            }`}
            {...register("shaba")}
            dir="ltr" // Right-to-left for IBAN numbers
          />
          {errors.shaba && (
        ...
        <div className="flex flex-row items-center w-full self-start gap-6 mt-auto ltr:justify-end rtl:justify-start">
          ...
          >
            {isSubmitting ? "در حال بررسی..." : "تایید شماره شبا"}
          </button>
        </div>
      </form>
    </div>
