...
const UploadPics = ({ apiUrl, adData, isEditing }) => {
  ...
  const handlePicsSubmit = (e) => {
    ...
    formData.append("ad_id", apiResponseData);

    savedImages.forEach((image, index) => {
      // Convert base64 → File
      const file = base64ToFile(image, `photo-${index}.jpg`);
      ...
      <div className="flex flex-wrap gap-4 items-center">
        ...
        {savedImages.map((img, index) => (
          ...
          >
            ...
            {mainImageIndex === index && (
              <div className="absolute bottom-1 left-1 right-1 bg-yellow-500/90 text-white text-xs px-2 py-1 rounded text-center font-medium">
                {u.main_pic}
              </div>
            )}
          </div>
