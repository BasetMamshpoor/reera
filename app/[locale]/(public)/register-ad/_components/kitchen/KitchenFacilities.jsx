...
const KitchenFacilities = ({ isEditing, adData }) => {
  const {
    handleSubmit,
    register,
    setValue,
    ...
    defaultValues: {
      condition: adData?.third?.condition || "",
      text: adData?.third?.text || "",
    },
  });
  ...
  const KitchenMutation = useMutation({
    mutationFn: async (data) => {
      if (!isEditing) {
        await request({
          url: "/store/kitchen/third",
          method: "post",
          data,
        });
      } else {
        await request({
          url: `/update/kitchen/third/${adData?.first?.id}`,
          method: "post",
          data,
        });
      }
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-row items-center w-full rtl:justify-end gap-6 mt-auto justify-end">
        ...
        >
          <span>{KitchenMutation.isLoading ? "در حال ارسال..." : "بعدی"}</span>
...
export default KitchenFacilities;
