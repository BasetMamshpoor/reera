...
import { CheckCircle2, Circle, Clock, AlertCircle, FileText, Download, Upload, Calendar, Bell, Target, ChevronRight, ChevronDown, Check, X, Star, TrendingUp } from 'lucide-react';
            ...
            <div className="max-w-6xl mx-auto px-4">
                ...
                <div className="bg-white rounded-2xl shadow-lg p-6 lg:p-8 mb-8">
                    <div className="flex items-center justify-between mb-4">
                        <div>
                            <h3 className="text-xl text-[#142738] font-medium">پیشرفت کلی</h3>
                            <p className="text-sm text-[#64656f] mt-1">
                        ...
                        <div className="text-left">
                            ...
                            <div className="text-xs text-[#64656f] mt-1">تکمیل شده</div>
                        </div>
                    </div>
                    ...
                    <div className="h-4 bg-gray-100 rounded-full overflow-hidden mb-4">
                        ...
                        >
                            {progressPercentage > 10 && (
                                <span className="text-xs text-white font-medium">{Math.round(progressPercentage)}%</span>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-3 gap-4">
                        ...
                        <div className="text-center p-4 bg-orange-50 rounded-xl">
                            <div className="text-2xl font-bold text-orange-600">{totalDocuments - completedDocuments}</div>
                ...
                <div className="flex items-center justify-between mb-6">
                    <button
                        ...
                        className="flex items-center gap-2 px-4 py-2 bg-white border-2 border-gray-200 rounded-xl hover:border-blue-300 transition-all"
                    >
                        <Check className={`w-4 h-4 ${showCompleted ? 'text-green-600' : 'text-gray-400'}`} />
                        <span className="text-sm text-[#142738]">نمایش انجام شده‌ها</span>
                    </button>

                    <div className="text-sm text-[#64656f]">
                        مسیر: <span className="font-medium text-[#142738]">{pathway}</span> → {country}
                    </div>
                </div>
                ...
                <div className="relative">
                    {/* Vertical Line */}
                    <div className="absolute right-6 lg:right-8 top-0 bottom-0 w-0.5 bg-gradient-to-b from-blue-200 via-green-200 to-purple-200"></div>
                        ...
                        {milestones.map((milestone) => {
                            ...
                            return (
                                <div key={milestone.id} className="relative pr-16 lg:pr-20">
                                    {/* Milestone Icon */}
                                    <div className={`absolute right-3 lg:right-5 w-12 h-12 rounded-full flex items-center justify-center text-2xl z-10 ${
                                        isMilestoneComplete
                            ...
                            <span className="inline-flex items-center gap-1 px-3 py-1 bg-blue-100 text-blue-700 rounded-full text-sm font-medium">
                              مرحله {milestone.phase}
                                                        ...
                                                        <span className="text-sm text-[#64656f]">{milestone.duration}</span>
                                                    </div>
                                                    <h3 className="text-xl text-[#142738] font-medium mb-1">{milestone.title}</h3>
                                                    <p className="text-sm text-[#64656f]">{milestone.description}</p>
                                                </div>

                                                <div className="flex items-center gap-4 mr-4">
                                                    <div className="text-center">
                                                        <div className={`text-2xl font-bold ${isMilestoneComplete ? 'text-green-600' : 'text-blue-600'}`}>
                                                            {Math.round(milestoneProgress)}%
                                                        </div>
                                                        <div className="text-xs text-[#64656f]">
                                                            {milestone.documents.filter(d => d.completed).length}/{milestone.documents.length}
                                                        </div>
                                                    </div>
                                                    <ChevronDown className={`w-6 h-6 text-[#64656f] transition-transform ${isExpanded ? 'rotate-180' : ''}`} />
                                                </div>
                                            </div>
                                            ...
                                            <div className="h-2 bg-gray-100 rounded-full overflow-hidden mt-4">
                                                <div
                                                    className={`h-full rounded-full transition-all ${
                                                        isMilestoneComplete ? 'bg-green-500' : 'bg-blue-500'
                                                    }`}
                                                                        ...
                                                                        <div className="flex items-start justify-between gap-4 mb-2">
                                                                            <div className="flex-1">
                                                                                <h4 className={`text-base font-medium mb-1 ${
                                                                                    doc.completed ? 'text-gray-500 line-through' : 'text-[#142738]'
                                                                                }`}>
                                                                                    {doc.title}
                                                                                </h4>
                                                                                <p className="text-sm text-[#64656f] leading-relaxed">
                                                                                    {doc.description}
                                                                                </p>
                                                                            </div>

                                                                            <div className="flex flex-col items-end gap-2">
                                        <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-medium border-2 ${
                                          ...
                                          <span className="hidden lg:inline">{doc.category}</span>
                                            ...
                                            <AlertCircle className="w-3 h-3" />
                                                                        ...
                                                                        {/* Meta Info */}
                                                                        <div className="flex items-center gap-4 text-xs text-[#64656f] mb-3">
                                                                        ...
                                                                        {doc.tips && doc.tips.length > 0 && !doc.completed && (
                                                                            <div className="bg-blue-50 border-r-4 border-blue-400 rounded-lg p-3 mb-3">
                                                                                <div className="flex items-start gap-2">
                                                                                    <Star className="w-4 h-4 text-blue-600 flex-shrink-0 mt-0.5" />
                                                                                    <div className="flex-1">
                                                                                        <div className="text-xs font-medium text-blue-900 mb-1">نکات مهم:</div>
                                                                        ...
                                                                        <div className="flex items-center gap-2">
                                                                            {doc.sampleLink && (
                                                                                <a
                                                                                    ...
                                                                                    className="inline-flex items-center gap-1 px-3 py-1.5 bg-purple-100 text-purple-700 rounded-lg text-xs hover:bg-purple-200 transition-colors"
                                                                                >
                                                                            ...
                                                                            <button className="inline-flex items-center gap-1 px-3 py-1.5 bg-gray-100 text-gray-700 rounded-lg text-xs hover:bg-gray-200 transition-colors">
                                        ...
                                        <textarea
                                            ...
                                            className="w-full px-3 py-2 text-sm border-2 border-gray-200 rounded-lg focus:border-blue-400 focus:outline-none resize-none"
                                            rows={2}
                                        />
                                                                            </div>
                                                                        )}
                                                ...
                                                {isMilestoneComplete && (
                                                    <div className="mt-6 bg-gradient-to-r from-green-50 to-emerald-50 border-2 border-green-300 rounded-xl p-4">
                                                        <div className="flex items-center gap-3">
                                                            ...
                                                            <div>
                                                                <div className="text-green-900 font-medium">مرحله {milestone.phase} تکمیل شد! 🎉</div>
                                                                <div className="text-sm text-green-700">می‌توانید به مرحله بعدی بروید</div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                )}
                ...
                {progressPercentage === 100 && (
                    <div className="mt-8 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-2xl shadow-2xl p-8 text-center">
                        <div className="text-6xl mb-4">🎊</div>
                        <h3 className="text-3xl font-bold mb-2">تبریک! همه مدارک آماده است!</h3>
                        <p className="text-green-100 text-lg mb-6">
                            شما آماده‌ی ثبت درخواست ویزا هستید
                        </p>
                        <button className="bg-white text-green-600 px-8 py-4 rounded-xl hover:shadow-xl transition-all transform hover:-translate-y-1 font-medium">
                ...
                <div className="mt-8 bg-gradient-to-r from-blue-50 to-indigo-50 border-2 border-blue-200 rounded-2xl p-6">
                    <div className="flex items-start gap-3">
                        ...
                        <div>
                            <h4 className="text-[#142738] font-medium mb-2">💡 یادآوری هوشمند</h4>
